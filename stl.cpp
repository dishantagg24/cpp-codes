...
void explainVector() {
	...
	bool present=binary_search(v.begin(),v.end(),value);	//for checking if value is present in a vector or not & time complexity is O(logn)
...
void explainSet() {
	//Set stores all unique elements in ascending order
	set<int>st;
	st.insert(1); // {1}
	st.emplace(2); // {1, 2}
	st.insert(2); // {1, 2}
	...
	//if we want to check that an element is present or not then use st.find(element)==st.end(). If this condition is true then element is not present.
...
void explainMap() {
    ...
    // This is the syntax
	auto it = mpp.lower_bound(2); 		//lower bound display the smallest or value equal to element(Here it is 2)

	auto it = mpp.upper_bound(3); 		//upper bound display the highest value to element
...
void explainExtra() {
	...
	sort(a+2, a+4); // [first, last) //for sorting in a particular range
	...
	int num = 7; // 111 
	int cnt = __builtin_popcount(); 	//count no. of set bits in a integer

	long long num = 165786578687;
	int cnt = __builtin_popcountll();	//count no. of set bits in a long long interger
	...
	int maxi = *max_element(a,a+n);		//to get maximum element in given range 
}
